...
<head>
	...
	<link type="text/css" rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/admin/qna/adminQnaList.css">
					...
					<script>
						$(function () {
							$('#searchBtn').click(function () {
								self.location = "./adminQnaList.reo" + '${pageMaker.makeQuery(1)}' + "&searchType=" + $("select option:selected").val() + "&keyword=" + encodeURIComponent($('#keywordInput').val());
							});
						});   
					</script>
				</div>
					...
					<ul class="pagination justify-content-center">
						<c:if test="${pageMaker.prev}">
							<li class="page-item"><a class="page-link"
									href="./adminQnaList.reo${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a>
							</li>
						</c:if>

						<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
							<li class="page-item <c:out value=" ${scri.page==idx ? 'active' : '' }" /> ">
							<a class="page-link" href="./adminQnaList.reo${pageMaker.makeSearch(idx)}">${idx}</a></li>
						</c:forEach>

						<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
							<li class="page-item"><a class="page-link"
									href="./adminQnaList.reo${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
						</c:if>
					</ul>
				</div>
			</div>
