...
<html lang="ko">
	<head>
		...
		<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/admin/pay/payInfo.js"></script>
		<link type="text/css" rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css">
		<link type="text/css" rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/admin/pay/payInfo.css">
	...
	<body>
		...
		<section>
			<div id="main" class="col-lg-10 offset-lg-2 col-md-12 col-12">
				<h2 class="title textCenter">내 결제 상세</h2>
				<div class="payInfo">
					<div class="col-sm-12">
						...
						<table class="infoContent">
							...
							<tr>
								<td>비고</td>
								<td id="payRemark">${payDTO.pay_state != '결제취소' ? payDTO.pay_remark : ''}</td>
							</tr>
							<tr>
								<td>결제번호</td>
						...
						<hr/>
						<div class="infoBtn">
							<c:if test="${payDTO.off_unit == '월' && payDTO.pay_state == '결제완료'}">
							<button id="contractBtn" class="btn btn-info btn-lg" onclick="contractOpen()">계약서 확인</button>
							</c:if>
							<c:if test="${payDTO.pay_state != '결제취소' && payDTO.pay_state != '결제대기'}">
							<button id="cancelBtn" class="btn btn-danger btn-lg" onclick="cancelModal()">결제취소</button>
							</c:if>
							<c:choose>
							<c:when test="${prev == null}">
							<button class="btn btn-primary btn-lg" onclick="location.href='payList.reo?pageNo=${param.pageNo == null ? 1 : param.pageNo}&fromDate=${param.fromDate}&toDate=${param.toDate}&searchType=${param.searchType}&search=${param.search}'">목록</button>
							</c:when>
							<c:otherwise>
